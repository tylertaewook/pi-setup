---
name: babysit-pr
description: Babysit a GitHub pull request after creation by continuously polling review comments, CI checks/workflow runs, and mergeability state until the PR is merged/closed or user help is required. Diagnose failures, retry likely flaky failures up to 3 times, auto-fix/push branch-related issues when appropriate, and keep watching open PRs so fresh review feedback is surfaced promptly. Use when the user asks the agent to monitor a PR, watch CI, handle review comments, or keep an eye on failures and feedback on an open PR.
---
...
Resolve these two values once, at the start of the session, before running any command below. Every `$SKILL_DIR` and `$DEFAULT_BRANCH` in this file refers to them.

```bash
SKILL_DIR="$HOME/.agents/skills/babysit-pr"
DEFAULT_BRANCH="$(gh repo view --json defaultBranchRef -q .defaultBranchRef.name)"
```

Never assume the default branch is `main` or `master` — read it. Some repos here mirror to a second remote; babysitting only ever touches `origin`.

## Related Skills

Use these skills as supporting procedures while babysitting:

- Read `../address-pr-comments/SKILL.md` and reuse its full comment-audit procedure to fetch review threads with GraphQL resolution state, top-level PR comments, and current code context. Babysitting itself is an explicit request to monitor feedback, so this audit is in scope even though the standalone skill is otherwise manually triggered. The mutation rules in this skill remain authoritative: do not automatically reply to or resolve other humans' threads.
- If a `create-pr-comment` skill is present in `<available_skills>`, read it before drafting any suggested or user-approved GitHub response and match its reply voice, while retaining the attribution prefixes required by this skill. If it is absent, write plain, short replies that state what changed and nothing more.
...
3. At startup, and whenever review activity changes, audit all published feedback using the `address-pr-comments` procedure: unresolved review threads, top-level PR comments, inline comments, and submitted reviews. Do not rely only on the watcher's surfaced delta; use GraphQL `isResolved` state so existing unresolved feedback is not missed.
4. Inspect the `actions` list in the JSON response.
...
7. If `process_review_comment` is present, inspect surfaced published review items and decide whether to address them.
8. If a review item is actionable and correct, patch code locally, commit, push, and then resolve the associated review thread only when allowed by the GitHub state mutation policy below.
9. Do not post replies to human-authored review comments/threads unless the user explicitly confirms the exact response. Draft any suggested or approved response using `create-pr-comment` when that skill is available. If a human review item is non-actionable, already addressed, or not valid, surface the item and recommended response to the user instead of replying on GitHub.
...
12. On every loop, look for newly surfaced review feedback before acting on CI failures or mergeability state, then verify mergeability / merge-conflict status (for example via `gh pr view`) alongside CI. Also `git fetch origin` and, if the branch is behind `origin/$DEFAULT_BRANCH`, merge it in, resolve conflicts, and push — see "Staying Current with origin/$DEFAULT_BRANCH".
...
It intentionally surfaces recognized reviewer-bot feedback (see `REVIEW_BOT_LOGIN_KEYWORDS` in the watcher) in addition to human reviewer feedback. Most unrelated bot noise should still be ignored.
For safety, the watcher only auto-surfaces trusted human review authors (for example repo OWNER/MEMBER/COLLABORATOR, plus the authenticated operator) and recognized review bots.
...
2. Commit with `pi: address PR review feedback (#<n>)`.
...
You can resolve review comment threads from the human who requested babysitting or from a recognized
review bot. When resolving, leave a comment prefixed with `[from pi]: ` and explain what changes
...
## Staying Current with the Default Branch

Keep the branch fresh instead of letting it drift:

- Run `git fetch origin` on every loop (cheap) and compare the head branch against `origin/$DEFAULT_BRANCH`.
- Whenever `origin/$DEFAULT_BRANCH` has new commits, merge it in right away: `git merge origin/$DEFAULT_BRANCH`. Don't wait for GitHub to report `CONFLICTING` or for a reviewer to ask for a rebase.
- Also merge immediately when the PR reports merge conflicts, a dirty/behind mergeable state, or a required "branch is out of date" check.
- Resolve merge conflicts yourself when the resolution is clear from both sides of the diff; keep the intent of both changes. Run the relevant tests/lint/typecheck after resolving.
- If a conflict is genuinely ambiguous (conflicting product decisions, unclear ownership of overlapping logic), stop and ask the user instead of guessing.
- Push often: after every merge, conflict resolution, and fix commit. Never accumulate a pile of local commits — a pushed branch is what CI and reviewers see.
- Commit message default for merges: `pi: merge origin/$DEFAULT_BRANCH into PR #<n>`.
- After each merge push, resume the watcher on the new SHA (restart `--watch` in the same turn if you interrupted it).
...
- Fetch `origin` frequently and keep the branch merged up to date with `origin/$DEFAULT_BRANCH`; push after each merge.
- Prefer merge over rebase/force-push on a PR branch under review.
...
- `pi: fix CI failure on PR #<n>`
- `pi: address PR review feedback (#<n>)`
- `pi: merge origin/$DEFAULT_BRANCH into PR #<n>`

## Monitoring Loop Pattern
Use this loop in a live pi session:

1. Run `--once`.
2. Read `actions`.
3. First check whether the PR is now merged or otherwise closed; if so, report that terminal state and stop polling immediately.
4. Check CI summary, new review items, and mergeability/conflict status, and whether the branch is behind `origin/$DEFAULT_BRANCH` (fetch, then merge + resolve + push if so).
...
- Fixes pushed
- Merges from `origin/$DEFAULT_BRANCH` and any conflicts resolved
